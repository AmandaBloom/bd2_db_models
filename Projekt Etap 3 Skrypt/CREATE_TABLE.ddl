...
CREATE SEQUENCE id_prod_seq START WITH 1000 INCREMENT BY 1 MAXVALUE 9999;

GRANT ALL PRIVILEGES ON id_prod_seq TO bd2c038_app;

CREATE SEQUENCE id_sys_seq START WITH 1000 INCREMENT BY 1 MAXVALUE 9999;
...
COMMENT ON TABLE certyfikacje IS
    'Certyfikacjê wystawia siê na dan¹ Wersjê Produktu na poszczególnych Platformach okreœlanych przez System Operacyjny oraz jego Numer Wersji. Dla ka¿dej Platformy ka¿da Wersja Produktu posiada Wartoœæ Certyfikacji - Certyfikowana "C", W Czasie Testów "T" lub Niecetyfikowana "N", zapisana jako jeden znak, posiada Datê Uzyskania tej Certyfikacji oraz moze posiadac Date Wygasniecia tej Certyfikacji';
...
COMMENT ON TABLE platformy IS
    'Platforma dotyczy Systemów Operacyjnych, ich numerów wersji, czyli kolejnych generacji. Platforma jest identfikowana przez ID Systemu (operacyjnego) oraz jego Numer Wersji. Platforma mo¿e zawieraæ opis. Platforma wchodzi w sk³ad Certyfikacji.';

COMMENT ON COLUMN platformy.numer_wersji IS
    'Identyfikator Wersji systemu operacyjnego dany jako liczba staloprzecinkowa zapisany jako ci¹g znaków np. "1404,02"';

COMMENT ON COLUMN platformy.opis IS
    'Nieobowiazkowy Opis Platformy Systemu Operacyjnego np. "WYKORZYSTUJE TRYB CIEMNY"';

GRANT SELECT, UPDATE, INSERT, DELETE ON platformy TO bd2c038_app;
...
COMMENT ON TABLE produkty IS
    'Produkt jest identyfikowany przez czterocyfrowy numer ID oraz Nazwê bêd¹c¹ ci¹giem znaków. Produkt mo¿e posiadaæ opis t³umacz¹cy np. jego zastosowanie, dostêpnoœæ oraz cechy.

Produkt wchodzi w sk³ad wersji produktu. ';
...
GRANT SELECT, INSERT, UPDATE, DELETE ON produkty TO bd2c038_app;
...
CREATE TABLE "Systemy Operacyjne" (
...
COMMENT ON TABLE "Systemy Operacyjne" IS
    'System Operacyjny jest identyfikowalny przez czterocyfrowy numer ID oraz Nazwê bêd¹c¹ ci¹giem znaków. Produkt mo¿e posiadaæ opis t³umacz¹cy np. jego cechy szczególne, dostêpnoœæ.

System Operacyjny wchodzi w sk³ad Platformy. ';

COMMENT ON COLUMN "Systemy Operacyjne".id_systemu IS
    'Unikalny Identyfikator systemu operacyjnego np. "1200"';

COMMENT ON COLUMN "Systemy Operacyjne".nazwa IS
    'Unikalna Nazwa Systemu Operacyjnego np. "Android OS 14.3"';

COMMENT ON COLUMN "Systemy Operacyjne".opis IS
    'Opis systemu operacyjnego np zawieraj¹cy cechy systemu np. "NIE POSIADA WSPARCIA DLA INTERFEJSU DOTYKOWEGO"';

GRANT SELECT, INSERT, UPDATE, DELETE ON "Systemy Operacyjne" TO bd2c038_app;

ALTER TABLE "Systemy Operacyjne" ADD CONSTRAINT sysop_pk PRIMARY KEY ( id_systemu );

ALTER TABLE "Systemy Operacyjne" ADD CONSTRAINT sysop_un UNIQUE ( nazwa );

CREATE TABLE "Wersje Produktu" (
    id_produktu  NUMBER(4) NOT NULL,
    numer_wersji VARCHAR2(7 CHAR) NOT NULL,
    opis         VARCHAR2(200 BYTE)
)
...
COMMENT ON TABLE "Wersje Produktu" IS
    'Wersja Produktu dotyczy Produktów ich wydañ, kolejnych generacji. Wersja Produktu jest identfikowana przez ID Produktu oraz Numer Wersji. Wersja produktu mo¿e zawieraæ opis.

Wersja Produktu wchodzi w sk³ad Certyfikacji.';

COMMENT ON COLUMN "Wersje Produktu".numer_wersji IS
...
COMMENT ON COLUMN "Wersje Produktu".opis IS
    'Opis wersji produktu np. "ZAWIERA FUNKCJONALNOSC TLUMACZENIA INTERFEJSU NA JEZYKI BALKANSKIE"';

GRANT SELECT, INSERT, UPDATE, DELETE ON "Wersje Produktu" TO bd2c038_app;

ALTER TABLE "Wersje Produktu" ADD CONSTRAINT wersje_produktu_pk PRIMARY KEY ( id_produktu,
                                                                              numer_wersji );
...
ALTER TABLE certyfikacje
    ADD CONSTRAINT cert_wersje_produktu_fk FOREIGN KEY ( id_produktu,
                                                         numer_wersji_produktu )
        REFERENCES "Wersje Produktu" ( id_produktu,
                                       numer_wersji )
    DEFERRABLE;

ALTER TABLE platformy
    ADD CONSTRAINT platformy_sysop_fk FOREIGN KEY ( id_systemu )
        REFERENCES "Systemy Operacyjne" ( id_systemu )
            ON DELETE CASCADE
    DEFERRABLE;

ALTER TABLE "Wersje Produktu"
...
CREATE OR REPLACE TRIGGER "FKNTM_Wersje Produktu" BEFORE
    UPDATE OF id_produktu ON "Wersje Produktu"
BEGIN
    raise_application_error(-20225, 'Non Transferable FK constraint  on table "Wersje Produktu" is violated');
END;
/
